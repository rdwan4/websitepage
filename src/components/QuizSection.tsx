...
import { DailyQuizBundle, LeaderboardEntry, Post } from "../types";
...
const copy = {
  en: {
    ...
    subtitle:
      "Five verified questions rotate each day from the Supabase question bank.",
    ready: "Ready to start?",
    startBody:
      "Answer today's 5 questions. Results are saved to your profile when you are signed in.",
    start: "Start Quiz",
    question: "Question",
    of: "of",
    finish: "Finish Quiz",
    ...
    minimumWarning:
      "Question bank below recommended minimum. Import 150+ verified questions.",
  ...
  ar: {
    ...
    ready: "هل أنت جاهز؟",
    startBody:
      "أجب عن أسئلة اليوم الخمسة. تحفظ النتائج في ملفك عند تسجيل الدخول.",
    start: "ابدأ الاختبار",
    question: "السؤال",
    of: "من",
...
export const QuizSection = ({
  ...
  const [latestCommunity, setLatestCommunity] = useState<Post | null>(null);
  ...
  const loadQuiz = async (
    ...
    setError("");
    try {
      const [quizBundle, leaderboardData, articlesData, communityData] =
        await Promise.all([
          contentService.getDailyQuiz(profile?.id, undefined, options),
          postService.getLeaderboard(3),
          postService.getApprovedPostsByCategorySlug("articles"),
          postService.getApprovedPostsByCategorySlug("community"),
        ]);

      setBundle(quizBundle);
      setLeaderboard(leaderboardData);
      setLatestArticle(articlesData[0] || null);
      setLatestCommunity(communityData[0] || null);

      const answerMap = Object.fromEntries(
        (quizBundle.answers || []).map((answer) => [
          answer.question_id,
  ...
  const handleAnswer = async (optionId: string) => {
    ...
    setSubmitting(true);
    try {
      const saved = await contentService.submitAnswer(
        profile.id,
        activeQuestion,
        optionId,
        bundle?.dateKey,
      ...
      setAnswers((current) => ({
        ...current,
        [activeQuestion.id]: saved.selected_option_id,
      }));
    } catch (saveError: any) {
            ...
            <div className="bg-app-card border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl relative overflow-hidden min-h-[440px] flex flex-col justify-center">
              ...
              {loading ? (
                ...
                <div className="text-center">
                  ...
                  <h3 className="mb-3 text-2xl font-bold text-app-text">
                    {t.empty}
                  </h3>
                </div>
              ) : (
                <AnimatePresence mode="wait">
                  {step === "start" && (
                    ...
                    >
                      ...
                      <h3 className="text-2xl font-bold text-app-text mb-4">
                        {t.ready}
                      </h3>
                      <p className="text-app-muted mb-8 max-w-sm mx-auto">
                        {t.startBody}
                      </p>
                    ...
                    >
                      ...
                      <div className="space-y-4 mb-8">
                        {activeQuestion.options.map((option) => {
                          ...
                          const isCorrect =
                            option.id === activeQuestion.correct_option_id;
                  ...
                  {step === "result" && (
                    ...
                    >
                      ...
                      <h3 className="text-3xl font-bold text-app-text mb-2">
                        {t.complete}
                      </h3>
                      <p className="text-app-muted mb-8">
                        {score} / {questions.length}.{" "}
                        {profile ? t.resultSignedIn : t.resultGuest}
                      </p>
                  ...
                  leaderboard.map((entry, index) => (
                    ...
                    >
                      ...
                      <div className="text-right">
                        <p className="text-lg font-bold text-app-accent">
                          {entry.total_score}
                        </p>
