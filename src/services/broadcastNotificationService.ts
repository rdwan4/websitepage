...
export const broadcastNotificationService = {
  async init() {
    ...
    try {
      ...
      await PushNotifications.addListener('registration', async (token) => {
        ...
        await this.syncTokenToProfile(token.value);
      });
  ...
  async deliverLocal(n: BroadcastNotification) {
    ...
    try {
      ...
      const title = n.title_ar || n.title_en;
      const body = n.body_ar || n.body_en;

      if (isNativeApp()) {
        const { isActive } = await CapacitorApp.getState();
        // Only show local banner if app is not active
        if (!isActive) {
          console.log('BNService: App is closed, showing local banner for:', n.id);
          await LocalNotifications.schedule({
            notifications: [{
              id: Math.abs(n.id.split('-').reduce((a, b) => a + b.charCodeAt(0), 0)) % 100000,
              title: title || 'Islamic Light',
  ...
  async syncPending() {
    ...
    const unseen = (data as BroadcastNotification[]).filter(n => {
      ...
      return sendTime > refDate;
    });

    console.log(`BNService: Found ${unseen.length} pending unreads.`);
    for (const n of unseen) {
      await this.deliverLocal(n);
    }
  }
};